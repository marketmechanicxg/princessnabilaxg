...
// dipanggil dari script.js setelah DOM siap
...
function spawnIntroSparkles() {
  ...
  const interval = setInterval(() => {
    ...
    s.style.cssText = `
      ...
      top:  ${Math.random() * 100}%;
      color: rgba(${Math.floor(Math.random()*80)+140},${Math.floor(Math.random()*60)+120},255,0.8);
...
function transitionToMain() {
  ...
  setTimeout(() => {
    // Show main site (model 4)
    main.classList.remove('hidden');
    main.classList.add('entering');

    // Boot model 4's full app
    window.App.init();
